...
router.get(
  "/:userId/exercises/:exerciseId/latest-workout",
  authMiddleware,
  async (req, res) => {
    try {
      ...
      if (rows.length === 0) {
        return res
          .status(404)
          .json({ error: "No recent workout found for this exercise." });
      }
...
router.post("/:userId/exercises", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (!exerciseResult.rows.length) {
      throw new Error(
        `Failed to find or insert exercise: ${normalizedExercise}`,
      );
    }
    ...
    res
      .status(201)
      .json({ message: "Exercise added successfully", exercise: exerciseId });
  } catch (error) {
    console.error("Database error:", error);
    res.status(500).json({ error: "Internal server error" });
  }
...
router.get(
  "/:userId/exercises/:exerciseId/volume-history",
  authMiddleware,
  async (req, res) => {
    try {
      const { userId, exerciseId } = req.params;
