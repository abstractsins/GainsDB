...
router.post("/:userId/log-workout", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    let exerciseId = exerciseResult.rows.length
    ...
    let workoutExerciseId = workoutExerciseResult.rows.length
      ? workoutExerciseResult.rows[0].id
      : null;
    ...
    res.status(201).json({
      message: "Workout set logged successfuly! LETS GOOOO!",
      setOrder,
    });
  } catch (error) {
