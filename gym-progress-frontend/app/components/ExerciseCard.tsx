...
const ExerciseCard: React.FC<Props> = ({ exercise, isExpanded, setExpandedExerciseId }) => {
    ...
    const handleClick = async (e: React.MouseEvent<HTMLLIElement>) => {
        ...
        if (!e.target.classList.contains('click-for-more')) {
            ...
            if (isExpanded) {
                ...
                setTimeout(() => setExpandedExerciseId(exercise.id), 300); // Expand the clicked card
            }
