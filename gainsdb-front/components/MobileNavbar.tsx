...
export default function Navbar({ sidebar }: Props) {
  ...
  const menuSlide = () =>
    sidebar.isMenuActive
      ? sidebar.setIsMenuActive(false)
      : sidebar.setIsMenuActive(true);
    ...
    <header className="mobile-nav-header">
      {username ? (
        ...
        >
          ...
          <h2
            className={`${oswald.className} text-lg font-bold  text-[12pt] sm:text-[14pt] md:text-[16pt] lg:text-[20pt]`}
          >
            Dashboard
          </h2>
        </Link>
      ...
      {username ? (
        <div className="hamburger-menu-container" onClick={menuSlide}>
          {" "}
          <RxHamburgerMenu className="text-[20pt]" />{" "}
        </div>
      ) : (
        <Link className="link" href="/">
          Login
